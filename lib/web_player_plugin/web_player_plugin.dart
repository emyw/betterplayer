...
import 'package:better_player/web_player_plugin/shaka_player.dart';
...
class BetterPlayerPlugin extends VideoPlayerPlatform {
  ...
  final Map<int, ShakaPlayer> _videoPlayers = <int, ShakaPlayer>{};
  ...
  void _disposeAllPlayers() {
    for (final ShakaPlayer videoPlayer in _videoPlayers.values) {
      videoPlayer.dispose();
    }
    _videoPlayers.clear();
  }
  ...
  Future<int?> create(
    ...
    ui.platformViewRegistry.registerViewFactory(
        'shakaVideoPlayer-$textureId', (int viewId) => videoElement);

    final ShakaPlayer player = ShakaPlayer(
  ...
  Future<int> setDataSource(int? textureId, DataSource dataSource) async {
    final ShakaPlayer player = _player(textureId!);
  ...
  // Retrieves a [ShakaPlayer] by its internal `id`.
  // It must have been created earlier from the [create] method.
  ShakaPlayer _player(int id) {
